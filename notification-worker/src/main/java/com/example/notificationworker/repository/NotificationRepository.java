package com.example.notificationworker.repository;
...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Modifying
    @Transactional
    @Query("UPDATE Notification n SET n.status = 'PROCESSING', n.processingStartedAt = :now WHERE n.id = :id AND n.status = 'QUEUED'")
    int claimForProcessing(@Param("id") Long id, @Param("now") LocalDateTime now);

    @Modifying
    @Transactional
    @Query("UPDATE Notification n SET n.status = 'QUEUED', n.processingStartedAt = NULL WHERE n.status = 'PROCESSING' AND n.processingStartedAt < :cutoff")
